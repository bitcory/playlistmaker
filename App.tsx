    ...
    <div className={`flex h-screen h-[100dvh] overflow-hidden transition-colors duration-300 ${
      ...
      {activeTab === 'project' ? (
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          {/* 헤더 - 모던 다크 스타일 */}
          <header
            className={`h-12 md:h-14 flex items-center justify-between px-3 md:px-4 border-b z-20 transition-colors duration-300 ${
              isDarkMode
          ...
          >
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {/* 로고 태그 */}
              <span className={`hidden sm:inline text-[10px] md:text-[11px] font-semibold tracking-widest px-2.5 py-1 rounded-md ${
                isDarkMode ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'bg-indigo-50 text-indigo-600 border border-indigo-200'
              }`}
              >STUDIO</span>
              <div className={`hidden sm:block h-2 w-2 rounded-full animate-pulse ${isDarkMode ? 'bg-emerald-400' : 'bg-emerald-500'}`} />
              <span className={`text-xs md:text-sm font-semibold tracking-wide ${isDarkMode ? 'text-zinc-300' : 'text-zinc-600'}`}>Preview</span>
            </div>
              ...
              className={`md:hidden p-2 rounded-lg transition-all duration-200 ${
                mobilePanel === 'right'
                  ...
                  : isDarkMode
                    ...
                    : 'bg-zinc-100 text-zinc-600 hover:bg-cyan-50 hover:text-cyan-600'
              }`}
            >
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            {/* 데스크톱: 좌측 패널 */}
            <div className={`hidden md:block w-[320px] border-r overflow-y-auto custom-scrollbar transition-colors duration-300 ${
              isDarkMode
            ...
            <main className={`flex-1 flex flex-col relative transition-colors duration-300 ${
              ...
              <div className="flex-1 p-2 md:p-3 flex flex-col gap-2 md:gap-3 overflow-hidden min-h-0">
                <div className={`w-full aspect-video max-h-full relative overflow-hidden rounded-xl group ${
                  isDarkMode
                    ? 'bg-black border border-zinc-800 shadow-2xl shadow-black/50'
              ...
              {/* Audio Deck - 모던 글래스모피즘 스타일 */}
              <div
                className={`h-16 md:h-20 rounded-xl flex items-center px-3 md:px-5 gap-2 md:gap-4 transition-colors duration-300 ${
                  isDarkMode
                    ? 'bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 shadow-lg shadow-black/20'
                    : 'bg-white/80 backdrop-blur-xl border border-zinc-200 shadow-md'
                }`}
              >
                {/* 이전 트랙 버튼 */}
                <button
                  ...
                  className={`w-9 h-9 md:w-11 md:h-11 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-200 disabled:opacity-30 ${
                    isDarkMode
                ...
                <button
                  ...
                  disabled={!audioSrc}
                  className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-200 disabled:opacity-30 ${
                    isPlaying
                ...
                <div className="flex-1 flex flex-col gap-2 md:gap-3 min-w-0">
                  ...
                  <div className="flex justify-between items-center gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className={`text-[10px] md:text-xs font-medium tracking-tight truncate ${isDarkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>
                        {projectAssets.audioTracks[currentTrackIndex]?.file?.name || 'No file selected'}
                      </span>
                    </div>
                    <span className={`text-[9px] md:text-[10px] font-medium tabular-nums tracking-tight flex-shrink-0 px-2 py-0.5 rounded-md ${
                  ...
                  {/* 프로그레스 바 */}
                  <div
                    className={`relative h-2 md:h-2.5 rounded-full overflow-hidden cursor-pointer transition-colors ${
                  ...
                  >
                    ...
                    {trackDurations.map((_, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`absolute top-0 bottom-0 w-px z-10 ${isDarkMode ? 'bg-zinc-600' : 'bg-zinc-400'}`}
                          style={{ left: `${position}%` }}
                        />
                      );
                    })}
                    {/* 진행 바 */}
                    <div
                      className="h-full rounded-full transition-all duration-150 bg-gradient-to-r from-indigo-600 to-cyan-500"
            ...
            {/* 데스크톱: 우측 패널 */}
            <div className={`hidden md:block w-[320px] border-l overflow-y-auto custom-scrollbar transition-colors duration-300 ${
              isDarkMode
                ? 'bg-zinc-900/50 border-zinc-800'
                : 'bg-white border-zinc-200'
            }`}>
          ...
          {mobilePanel !== 'none' && (
            <div className="md:hidden fixed inset-0 z-50 flex">
              ...
              {/* 패널 */}
              <div className={`relative w-[85%] max-w-[320px] h-full overflow-y-auto transition-colors duration-300 ${
                mobilePanel === 'right' ? 'ml-auto border-l' : 'border-r'
              } ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
                {/* 패널 헤더 */}
                <div className={`sticky top-0 z-10 flex items-center justify-between p-4 border-b transition-colors duration-300 ${
                  isDarkMode ? 'bg-zinc-900/90 backdrop-blur-xl border-zinc-800' : 'bg-white/90 backdrop-blur-xl border-zinc-200'
                }`}>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-lg transition-all duration-200 ${
                      isDarkMode ? 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200' : 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200 hover:text-zinc-800'
                    }`}
                  >
                    <X className="w-4 h-4" />
                  </button>
      ...
      {/* 모바일 하단 네비게이션 */}
      <div className={`md:hidden fixed bottom-0 left-0 right-0 h-16 border-t flex items-center justify-around z-40 transition-colors duration-300 ${
        isDarkMode
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 px-5 py-2 rounded-lg transition-all duration-200 ${
            activeTab === 'project'
              ? isDarkMode
